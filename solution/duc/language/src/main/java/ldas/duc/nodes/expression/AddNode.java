 ...
 * are automatically generated by the Truffle DSL. In addition, a {@link AddNodeGen factory class}
 * is generated that provides, e.g., {@link AddNodeGen#create node creation}.
 */
@NodeInfo(shortName = "+")
public abstract class AddNode extends BinaryNode {
     ...
     * the {@link #add(BigNumber, BigNumber) slow path}. Therefore, we use an
    ...
    /**
     * This is the slow path of the arbitrary-precision arithmetic. The {@link BigNumber} type of
     * Java is doing everything we need.
     * <p>
     * This specialization is automatically selected by the Truffle DSL if both the left and right
     * operand are {@link BigNumber} values. Because the type system defines an
     * {@link ImplicitCast implicit conversion} from {@code long} to {@link BigNumber} in
    ...
    @Specialization
    @TruffleBoundary
    protected BigNumber add(BigNumber left, BigNumber right) {
        return new BigNumber(left.getValue().add(right.getValue()));
    }
