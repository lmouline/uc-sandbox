...
/**
 * Duc is a simple language to demonstrate and showcase features of Truffle. The implementation is as
 ...
 * <p>
 * Duc is dynamically typed, i.e., there are no type names specified by the programmer. Duc is
 ...
 * calls to the {@link DucNodeFactory} that performs the actual node creation. All functions found in
 * the Duc source are added to the {@link DucFunctionRegistry}, which is accessible from the
 * {@link Context}.
 *
 * <p>
 * <b>Builtin functions:</b><br>
 * Library functions that are available to every Duc source without prior definition are called
 * builtin functions. They are added to the {@link DucFunctionRegistry} when the {@link Context} is
 * created. Some of the current builtin functions are
 * <ul>
 * <li>{@link DucReadlnBuiltin readln}: Read a String from the {@link Context#getInput() standard
 * input}.
 * <li>{@link DucPrintlnBuiltin println}: Write a value to the {@link Context#getOutput() standard
 * output}.
 * <li>{@link DucNanoTimeBuiltin nanoTime}: Returns the value of a high-resolution time, in
 * nanoseconds.
 * <li>{@link DucDefineFunctionBuiltin defineFunction}: Parses the functions provided as a String
 * argument and adds them to the function registry. Functions that are already defined are replaced
 * with the new version.
 * <li>{@link DucStackTraceBuiltin stckTrace}: Print all function activations with all local
 * variables.
 * </ul>
 */
@TruffleLanguage.Registration(id = DucLanguage.ID, name = "Duc", version = "1.0", mimeType = DucLanguage.MIME_TYPE)
...
public final class DucLanguage extends TruffleLanguage<Context> {
    ...
    /*
     * Still necessary for the old Duc TCK to pass. We should remove with the old TCK. New language
