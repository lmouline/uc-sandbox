...
public class SimpleTestRunner extends ParentRunner<SimpleTestRunner.TestCase> {

    private static final String SOURCE_SUFFIX = ".duc";
    ...
    public SimpleTestRunner(Class<?> runningClass) throws InitializationError {
        super(runningClass);
        try {
    ...
    protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
        DucTestSuite suite = c.getAnnotation(DucTestSuite.class);
        if (suite == null) {
            throw new InitializationError(String.format("@%s annotation required on class '%s' to run with '%s'.", DucTestSuite.class.getSimpleName(), c.getName(), SimpleTestRunner.class.getSimpleName()));
        }

        String[] paths = suite.value();

        Class<?> testCaseDirectory = c;
        if (suite.testCaseDirectory() != DucTestSuite.class) {
    ...
    public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
        ...
        SimpleTestRunner suite = new SimpleTestRunner(testClass);
        if (args.length > 0) {
